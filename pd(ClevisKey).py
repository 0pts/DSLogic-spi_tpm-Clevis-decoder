...
class Ann:
    """ Annotation ID """
    READ, WRITE, ADDRESS, WAIT, DATA, VMK, CLEVIS_KEY = range(7)
    ...
    def _find_clevis_key(self):
        ...
        if rs_candidate:
            ...
            if record_size < 0x200 and len(data) >= record_size*2:
                kdata = re.findall(rf'00000072{rsize}' + r'(\w{' + rf'{record_size*2}' + '})', data)
                if kdata:
                    try:
                        jkdata = json.loads(bytes.fromhex(kdata[0]))
                        if 'alg' in jkdata and 'k' in jkdata:
                            calg = jkdata['alg']
                            ckey = self.base64url_decode(jkdata['k']).hex()
                            print(f'[+] Found clevis-tpm2 key: {ckey}\n[i] Algorithm: {calg}')
                            self.put(self.ss, self.es, self.out_ann, [Ann.CLEVIS_KEY, [f'Clevis key found: {ckey}', f'Algorithm: {calg}']])
                            self.window = b''
