...
WINDOW_SIZE = 0x300  # Размер окна для хранения данных

class Ann:
    """ Annotation ID """
    READ, WRITE, ADDRESS, WAIT, DATA, VMK, CLEVIS_KEY, CLEVIS_JSON = range(8)
...
class Decoder(srd.Decoder):
    ...
    desc = 'Parses TPM transactions from SPI bus with automatic VMK and Clevis key extraction.'
    license = 'gplv2+'
    inputs = ['spi']
    outputs = []
    tags = ['IC', 'TPM', 'BitLocker', 'Clevis']
    annotations = (
        ...
        ('Clevis Key', 'Extracted Clevis TPM2 Key'),
        ('Clevis JSON', 'Clevis Key JSON Data'),
    )
    annotation_rows = (
        ('Transactions', 'TPM transactions', (0, 1, 2, 3, 4)),
        ('B-VMK', 'BitLocker Volume Master Key', (5,)),
        ('Clevis Key', 'Clevis TPM2 Key', (6,)),
        ('Clevis JSON', 'Clevis Key JSON Data', (7,)),
    )
    options = (
    ...
    def __init__(self):
        ...
        self.window = b''  # Буфер для хранения данных транзакций
        self.window_samples = []  # Список для хранения пар (начальный, конечный) сэмплов для данных в окне
    ...
    def _transaction_data(self, mosi, miso):
        ...
        if annotation:
            ...
            # Добавляем данные в окно и ищем ключ Clevis TPM2
            if self.current_transaction.operation == Operation.READ and self._is_fifo_transaction():
                self.window += self.current_transaction.data
                self.window_samples.append((self.current_transaction.start_sample, self.current_transaction.end_sample_data))
                self._find_clevis_key()
                if len(self.window) > WINDOW_SIZE:
                    self.window = self.window[-WINDOW_SIZE:]
                    self.window_samples = self.window_samples[-WINDOW_SIZE:]  # Удаляем старые сэмплы

            self.end_current_transaction()

    def _is_fifo_transaction(self):
        try:
    ...
    def _find_clevis_key(self):
        data = self.window.hex()
        rs_candidate = re.findall(r'00000072(\w{4})', data)
        if rs_candidate:
            rsize = rs_candidate[0]
            record_size = int(rsize, 16)
            if record_size < 0x200 and len(data) >= (record_size*2 + 12):  # +12 для учета заголовка
                pattern = rf'00000072{rsize}(\w{{{record_size*2}}})'
                kdata_match = re.search(pattern, data)
                if kdata_match:
                    kdata_hex = kdata_match.group(1)
                    try:
                        jkdata_bytes = bytes.fromhex(kdata_hex)
                        jkdata = json.loads(jkdata_bytes)
                        if 'alg' in jkdata and 'k' in jkdata:
                            calg = jkdata['alg']
                            ckey = self.base64url_decode(jkdata['k']).hex()
                            print(f'[+] Found clevis-tpm2 key: {ckey}\n[i] Algorithm: {calg}')
                            # Получаем сэмплы для аннотации
                            start_sample = self.window_samples[0][0]
                            end_sample = self.window_samples[-1][1]
                            # Выводим ключ Clevis
                            self.put(start_sample, end_sample, self.out_ann, [Ann.CLEVIS_KEY, [f'Clevis key found: {ckey}', f'Algorithm: {calg}']])
                            # Выводим весь JSON в отдельной строке аннотаций
                            json_str = jkdata_bytes.decode('utf-8', errors='ignore')
                            self.put(start_sample, end_sample, self.out_ann, [Ann.CLEVIS_JSON, [f'Clevis JSON: {json_str}']])
                            self.window = b''
                            self.window_samples = []
